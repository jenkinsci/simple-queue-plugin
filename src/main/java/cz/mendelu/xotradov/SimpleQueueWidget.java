...
public class SimpleQueueWidget extends Widget {
    ...
    private ItemGroup viewOwner;

    public SimpleQueueWidget(@NonNull String ownerUrl, @NonNull List<QueueItem> queueItems, boolean filtered, String viewName, ItemGroup viewOwner) {
    ...
    @SuppressWarnings("unused") // jelly
    public static String getViewOwnerParamName(){return MoveAction.VIEW_OWNER_PARAM_NAME;}
    ...
    public static final class ViewFactoryImpl extends WidgetFactory<View, SimpleQueueWidget> {
        ...
        public Collection<SimpleQueueWidget> createFor(@NonNull View target) {
            return List.of(new SimpleQueueWidget(target.getUrl(), new ArrayList<>(target.getQueueItems()), target.isFilterQueue(), target.getViewName(), target.getOwner().getItemGroup()));
        }
    }
